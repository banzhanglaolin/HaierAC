...
When the config flow opens, it first sends a UDP broadcast discovery request to `255.255.255.255:7083`. If a device responds, the integration parses the returned device name, IP address, MAC address, module type, and firmware string. The IP address and MAC address fields are prefilled from the response, and the discovered module string is shown as read-only text in the form description. You can still enter the values manually.
